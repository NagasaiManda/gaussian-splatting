...
def load_lr_gt(viewpoint_cam, dataset_path):
    ...
    for ext in [".jpg", ".png", ".JPG", ".PNG"]:
        path = os.path.join(lr_dir, f"{base_name}{ext}")
        if os.path.exists(path):
            return transforms.ToTensor()(Image.open(path).convert("RGB")).to("cuda")

    raise FileNotFoundError(f"Missing LR image for {base_name}")
